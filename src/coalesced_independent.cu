...
// next_frontier
#define BUFFER_SIZE 400 // Adjusted for shared memory size
__global__ void bfs_kernel_coalesced_independent(
  ...
  uint32_t flush_size = min(buffer_size, BUFFER_SIZE);
  uint32_t start_index = atomicAdd(next_frontier_size, flush_size);
  for (uint32_t i = 0; i < flush_size; i++) {
      //uint32_t index = atomicAdd(next_frontier_size, 1);
      next_frontier[start_index+i] = buffer[i];
    }
...
void gpu_bfs_coalesced_independent(const uint32_t N, const uint32_t M,
  ...
  while (current_frontier_size > 0) {
    ...
    bfs_kernel_coalesced_independent<<<grid_dim, block_dim>>>(
