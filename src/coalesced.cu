...
/*__global__ void bfs_kernel_coalesced(
  ...
  for (uint32_t j = 0; j < block_size; j+=1) {
    ...
    uint32_t tid = blockIdx.x * blockDim.x + j;
    if (tid >= frontier_size)
      return;
    __syncthreads(); // Ensure all threads are synchronized before accessing shared memory 
    uint32_t node = frontier[tid];
    uint32_t row_start = row_offsets[node];
    uint32_t row_end = row_offsets[node + 1];

    for (uint32_t i = row_start+threadIdx.x; i < row_end; i+=block_size) {
